...
Script to explore supported wallpaper sites for available themes, resolutions, and categories.
Generates a SITES.md report to help with configuration decisions.
"""
...
class WallpaperScout:
    """
    Explores wallpaper sites for themes, resolutions, and categories.
    Generates a markdown report for user configuration.
    """

    def __init__(self):
        """
        Initialize the scout with HTTP headers and config values.
        """
        self.headers = {
    ...
    def scout_wallpaperswide(self):
        """
        Scout information from wallpaperswide.com (themes, resolutions, categories).
        """
        site_info = {
            'name': 'WallpapersWide',
        ...
        # If no themes were found, add some common themes that are known to
        # work
    ...
    def scout_wallhaven(self):
        """
        Scout information from wallhaven.cc (themes, resolutions, categories).
        """
        site_info = {
            'name': 'Wallhaven',
        ...
        if resp:
            soup = BeautifulSoup(resp.text, 'html.parser')
            resolution_options = soup.select(
                'select[name="resolutions"] option')
            for option in resolution_options:
                res_text = option.text.strip()
                if res_text and 'x' in res_text and not res_text.startswith(
                        'At least'):
                    site_info['resolutions'].append(res_text)

        # If no themes were found, add some common tags that are known to work
        # on wallhaven
        if not site_info['themes']:
            common_themes = [
    ...
    def scout_wallpaperbat(self):
        """
        Scout information from wallpaperbat.com (themes, resolutions, categories).
        """
        site_info = {
            'name': 'WallpaperBat',
        ...
        for item in search_items:
            ...
            if search_text and len(
                    search_text) > 2 and search_text not in popular_searches:
        ...
        # Clean up themes
        site_info['themes'] = sorted(
            list(set([t for t in site_info['themes'] if len(t) > 2])))
    ...
    def generate_report(self, sites_info):
        """
        Generate a markdown report with the collected information for all sites.
        """
...
"""
        ...
        # Add common themes section - gather themes that appear in multiple
        # sites
        all_themes = {}
        for site in sites_info:
        ...
        if common_themes:
            ...
            sorted_common = sorted(
                common_themes, key=lambda x: (-x['count'], x['name'].lower()))
        ...
        for site in sites_info:
            for res in site['resolutions']:
                # Extract just the dimensions, ignore aspect ratio prefix if
                # present
        ...
        for site in sites_info:
            ...
            if site['resolutions']:
                ...
                res_chunks = [site['resolutions'][i:i + 5]
                              for i in range(0, len(site['resolutions']), 5)]
            ...
            if site['themes']:
                ...
                theme_chunks = [site['themes'][i:i + 5]
                                for i in range(0, len(site['themes']), 5)]
...
"""
    ...
    def run(self):
        """
        Run the scout on all supported sites and generate the report.
        """
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for future in tqdm(
                ...
                try:
                    ...
                    logging.info(
                        f"Found {len(site_info['themes'])} themes and {len(site_info['resolutions'])} resolutions on {site_name}")
...
if __name__ == "__main__":
    try:
        ...
        success = scout.run()
        if success:
            logging.info(f"✅ Successfully created {scout.output_file}")
            logging.info(f"   Path: {os.path.abspath(scout.output_file)}")
        else:
            logging.error(f"❌ Failed to create {scout.output_file}")
    except Exception as e:
        logging.error(f"❌ Error running wallpaper scout: {e}")
        import traceback
        traceback.print_exc()
